...
def ll_no_events(w_i, b_i, g_i, tn_ti, t_ti):
    return torch.sum(0.5 * np.sqrt(np.pi) * g_i * w_i * (torch.erf((b_i - t_ti) / g_i)
    - torch.erf((b_i - tn_ti) / g_i)), -1)
...
def t_intensity(w_i, b_i, g_i, t_ti):
    v_i = w_i * torch.exp(-((t_ti - b_i) / g_i)**2)
